        ...
        <div style={{ marginTop: 14 }}>
          {results.map(user => (
            <div key={user.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0', borderBottom: '1px solid #2a2b3d' }}>
              <img src={user.avatar || '/default-avatar.png'} alt="avatar" style={{ width: 36, height: 36, borderRadius: '50%', objectFit: 'cover', background: '#222' }} />
              <div style={{ flex: 1 }}>
                <div style={{ color: '#e3e8f0', fontWeight: 500 }}>{user.login}</div>
