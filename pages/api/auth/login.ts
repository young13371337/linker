...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (user.twoFactorToken) {
      if (!token) return res.status(200).json({ require2FA: true, user: { id: user.id, login: user.login } });
      if (token !== user.twoFactorToken) return res.status(401).json({ error: "Invalid 2FA token" });
    }
