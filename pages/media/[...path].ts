...
// This route serves media files from the storage directory under a unified
// /media/* URL. Expected URL patterns:
// /media/linker/video/<filename>
// /media/linker/voice/<filename>
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    console.log('[MEDIA PAGE] request for', req.url, '->', fullPath);

    if (!fs.existsSync(fullPath)) {
      console.warn('[MEDIA PAGE] File not found:', fullPath);
      res.status(404).end();
      return;
    }
    ...
    console.error('[MEDIA PAGE] Error serving media:', e);
